...
class ChatCompletionRequest(BaseModel):
    ...
    conversation_id: Optional[str] = Field(default=None, description="Persistent conversation thread ID")
...
# Persistent conversation ID storage
CONVERSATION_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".hermes_conversation_ids.json")
...
def save_conversation_ids(ids_dict):
    try:
        with open(CONVERSATION_FILE, "w") as f:
            json.dump(ids_dict, f)
    except IOError as e:
        logger.warning(f"Failed to save conversation IDs: {e}")

def get_conversation_id(user: str) -> str:
    ids = load_conversation_ids()
    if not user:
        user = "default"
    if user not in ids:
        ids[user] = str(uuid.uuid4())
        save_conversation_ids(ids)
        logger.info(f"Created new conversation thread for user: {user} -> {ids[user]}")
    return ids[user]
...
async def make_1minai_request(messages: List[ChatMessage], model: str, temperature: float = 0.7, max_tokens: Optional[int] = None, response_format: Optional[Dict[str, Any]] = None, top_p: Optional[float] = 1.0, stop: Optional[List[str]] = None, tools: Optional[List[Tool]] = None, tool_choice: Optional[str] = "auto", conversation_id: Optional[str] = None) -> Dict[str, Any]:
    """
...
async def chat_completions(
    ...
    try:
        ...
        # Resolve or create persistent conversation thread
        user_key = request.user or "default"
        if not request.conversation_id:
            request.conversation_id = get_conversation_id(user_key)
        logger.info(f"Conversation thread: {request.conversation_id} (user={user_key})")
